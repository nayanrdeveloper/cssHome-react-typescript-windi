...
export default function AdminLogin() {
  ...
  const onSubmitHandler = (event: any) => {
    ...
    if (password && email) {
      axios
        .post("http://localhost:5000/user/admin/login", userLoginData)
        .then((result) => {
          if (result.status == 200) {
            setWarning("User Login Successfully!!!");
            setUserLoginData({
              password: "",
              email: "",
            });
            ...
            localStorage.setItem('is_admin', JSON.stringify(result.data.user.is_admin))
            // localStorage.setItem('is_admin', JSON.stringify(result.data.user))
            const login = localStorage.getItem('user')
            const user = ''
            if (login){
              const user = JSON.parse(login)
              StoreData.setLoginUser(user.name)
            }
